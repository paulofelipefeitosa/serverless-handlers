...
```{r bpftrace_data_treatment, fig.width=7, fig.height=5}
require(dplyr)

read_bpftrace_pure_data <- function(f, app_name) {
  df <- read.csv(f)
  df$App <- app_name
  df <- df %>% rename(Value_NS = KernelTime_NS)
  
  clone <- df[which(df$Metric == "CloneEntry"), ]
  clone$Value_NS <- df[which(df$Metric == "CloneExit"), ]$Value_NS - clone$Value_NS
  clone$Metric <- "CloneTime"
  
  exec <- df[which(df$Metric == "CloneExit"), ]
  exec$Value_NS <- df[which(df$Metric == "ExecveExit"), ]$Value_NS - exec$Value_NS
  exec$Metric <- "ExecTime"
  
  runtime <- df[which(df$Metric == "ExecveExit"), ]
  runtime$Value_NS <- df[which(df$Metric == "MainEntry"), ]$Value_NS - runtime$Value_NS
  runtime$Metric <- "RuntimeTime"
  
  appinit <- df[which(df$Metric == "MainEntry"), ]
  appinit$Value_NS <- df[which(df$Metric == "Ready2Serve"), ]$Value_NS - appinit$Value_NS
  appinit$Metric <- "InitializationTime"
  
  return(rbind(clone, exec, runtime, appinit))
}

nocriu_newbpftrace_nogc_nowarmup <- 
  rbind(read_bpftrace_pure_data('../server-http-handler-noop-1565746178-1000-1---.csv', "NOOP"),
        read_bpftrace_pure_data('../server-http-handler-thumbnailator-1565727388-1000-1---.csv', "Thumbnail Maker"))

write.csv(nocriu_newbpftrace_nogc_nowarmup, 
  file = "nocriu_newbpftrace_nogc_nowarmup.csv",
  row.names = FALSE)
```
...
read_startup_bpfstrace <- function(f) {
  ...
  df$value <- df$value / 10^6
  df <- df %>% mutate(app = ifelse(app == "NoOp" | app == "NOOP", "NOOP", "Thumbnail Maker"))
...
vanilla <- read_startup_bpfstrace("nocriu_newbpftrace_nogc_nowarmup.csv")
...
startup_components <- 
  rbind(noop_v_perc %>%
        group_by(app, type) %>%
        mutate(value = (perc/100.0) * median(vanilla_noop$value)),
      noop_pb_perc %>%
        group_by(app, type) %>% 
        mutate(value = (perc/100.0) * median(pb_noop$value)) %>%
        filter(metric != "RTS"), #%>%
        #mutate(metric = replace(metric, metric=="APPINIT", "RESTORE")),
      tm_v_perc %>%
        group_by(app, type) %>%
        mutate(value = (perc/100.0) * median(vanilla_thumb$value)),
      tm_pb_perc %>%
        group_by(app, type) %>%
        mutate(value = (perc/100.0) * median(pb_thumb$value)) %>%
        filter(metric != "RTS"))# %>%
        #mutate(metric = replace(metric, metric=="APPINIT", "RESTORE")))

ggplot(startup_components) +
  facet_wrap(~app) +
  geom_bar(aes(y = value, 
               x = type, 
               fill = factor(metric, c("RESTORE", "APPINIT", "RTS", "EXEC", "CLONE"))), 
           stat = "identity") +
  labs(fill = "Metric", x = "Technique", y = "Startup Time (ms)") +
  #theme_pubclean() +
  theme_bw() +
  scale_fill_grey(start = 0, end = 0.85) +
  theme(panel.grid.major.y = element_line(colour = "darkgray", linetype = 3))

ggsave("stacked_startup_components_bar_plot.png")
...
vanilla <- read_startup_time("nocriu-nobpftrace-nogc-nowarmup-noop-class-loader.csv")
vanilla$type <- "Vanilla"
